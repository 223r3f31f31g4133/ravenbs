...
public class RenderUtils implements IMinecraftInstance {
    ...
    public static void renderBoxV2(double x, double y, double z, double x2, double y2, double z2, int color, boolean outline, boolean shade, Vec3 lastTick) {
        float partialTicks = ((IAccessorMinecraft) mc).getTimer().renderPartialTicks;

        double xPos = x - lastTick.xCoord + (x - lastTick.xCoord) * (double) partialTicks - mc.getRenderManager().viewerPosX;
        double yPos = y - lastTick.yCoord + (y - lastTick.yCoord) * (double) partialTicks - mc.getRenderManager().viewerPosY;
        double zPos = z - lastTick.zCoord + (z - lastTick.zCoord) * (double) partialTicks - mc.getRenderManager().viewerPosZ;
        GL11.glPushMatrix();
        GL11.glBlendFunc(770, 771);
        glEnable(3042);
        GL11.glLineWidth(2.0f);
